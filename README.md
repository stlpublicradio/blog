...
View the [sample copy spreadsheet](https://docs.google.com/spreadsheet/pub?key=11b2jes_fUmTQcTjMGbMqfNlOTJfn6wFhUAZoQdq7zEs#gid=0).
...
def read_my_google_doc():
    ...
    get_document('11b2jes_fUmTQcTjMGbMqfNlOTJfn6wFhUAZoQdq7zEs', file_path)
    data = Copy(file_path)
...
```
* * * * * ubuntu bash /home/ubuntu/apps/blog/repository/cron.sh fab $DEPLOYMENT_TARGET cron_jobs.test
```
...
|--------|------|-----|-----|
|blog|tweet|`location`||
|blog|facebook|`location`||
|blog|email|`location`||
|blog|new-comment||
|blog|open-share-discuss||
|blog|close-share-discuss||
|blog|summary-copied||
|blog|featured-tweet-action|`action`|
|blog|featured-facebook-action|`action`|
